...
namespace Standups.Areas.Admin.Controllers
{
    ...
    public class CommentsApiController : ControllerBase
    {
        ...
        public ActionResult<List<CommentAdmin>> Comments(string status)
        {
            ...
            IEnumerable<Supcomment> result = null;
            switch(status.ToLower())
            {
                case "all":
                    result = _supcommentRepository.GetAll().ToList();
                    break;
                case "new":
                    result = _supcommentRepository.GetAll().Where(c => c.AdvisorRating == 0).ToList();
                    break;
                case "approved":
                    result = _supcommentRepository.GetAll().Where(c => c.AdvisorRating == 1).ToList();
                    break;
                case "rejected":
                    result = _supcommentRepository.GetAll().Where(c => c.AdvisorRating == -1).ToList();
                    break;
