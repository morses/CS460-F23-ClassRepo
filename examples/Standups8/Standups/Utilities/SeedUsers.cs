    ...
    public static class SeedUsers
    {
        ...
        public static async Task Initialize(IServiceProvider serviceProvider, UserInfoData[] seedData, string testUserPw )
        {
            try
            {
                ...
                using (var context = new StandupsDbContext(serviceProvider.GetRequiredService<DbContextOptions<StandupsDbContext>>()))
                {
                    ...
                    foreach(var u in seedData)
                    {
                        ...
                        Supuser su = new Supuser { AspnetIdentityId = identityID, FirstName = u.FirstName, LastName = u.LastName };
                        if(!context.Supusers.Any(x => x.AspnetIdentityId == su.AspnetIdentityId && x.FirstName == su.FirstName && x.LastName == su.LastName))
                        {
        ...
        public static async Task InitializeAdmin(IServiceProvider serviceProvider, string email, string userName, string adminPw, string firstName, string lastName)
        {
            try
            {
                using (var context = new StandupsDbContext(serviceProvider.GetRequiredService<DbContextOptions<StandupsDbContext>>()))
                {
                    ...
                    Supuser su = new Supuser { AspnetIdentityId = identityID, FirstName = firstName, LastName = lastName };
                    if (!context.Supusers.Any(x => x.AspnetIdentityId == su.AspnetIdentityId && x.FirstName == su.FirstName && x.LastName == su.LastName))
                    {
                        // Doesn't already exist, so add a new user
                        context.Add(su);
