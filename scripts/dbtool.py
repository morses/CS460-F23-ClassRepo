...
def split_sql(sql):
    # Attempt to match semicolons that are not inside single quotes
    # We would need to use a SQL parser to handle all cases.  Use a DRY RUN to see how it split them
    pattern = r";(?=(?:[^']*'[^']*')*[^']*$)"
    # Split the string on the regex pattern and remove any empty strings from the list
    return [s for s in re.split(pattern, sql) if s]
